...
import gr.james.influence.util.*;
...
public interface Graph<V, E> extends Iterable<V>, Metadata {
    ...
    default GraphEdge<V, E> addEdge(V source, V target, double weight, EdgeProvider<E> edgeProvider) {
        return addEdge(source, target, edgeProvider.get(), weight);
    }
     ...
     *
     * @param among        a collection of vertices of which each pair will be connected; you should prefer a collection
     *                     with a fast iterator implementation
     * @param edgeProvider the edge provider to use when generating a new edge instance
     ...
     */
    @AutoGenerated
    default void addEdges(Collection<V> among, EdgeProvider<E> edgeProvider) {
        for (V v : among) {
            for (V u : among) {
                if (!v.equals(u)) {
        ...
        assert Graphs.subGraph(this, MemoryGraph::new, among)
